        ...
        while True:
            ...
            if ask=="1":
                if method=="kali":
                   ...
                   threading.Thread(target=run_on_browser.main, args=self.command,).start() 
                elif method=="github":
                    if os.path.exists(f"Tools/{self.github_check}"):
                        ...
                        installed=input(f"{colors.blue}Do You Want To Install The Tool?(y/n):{colors.reset}")
...
def deb_install(name,command,link):
    proc = subprocess.Popen([f"dpkg -s {command} 2>/dev/null"], stdout=subprocess.PIPE, shell=True)
    (there, notthere) = proc.communicate()
    if "install ok installed" not in there.decode():
        print(f"\n{colors.red}[+] Not Installed")
        print(f"{colors.red}\n[+] It Is Not Installed In Your Kali{colors.reset}")
        ...
        if install.lower()=="yes" or install.lower()=="y":
            ...
            install = input(f"{colors.blue}Do you want to run the tool?(y/n):{colors.reset}")
